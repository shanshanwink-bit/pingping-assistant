...
test('Web 和小程序商品表单提供真实货号交互且不把 code 当货号', () => {
  ...
  const miniDetail = fs.readFileSync(path.join(root, 'pages/product-detail/index.wxml'), 'utf8')
  const miniDetailLogic = fs.readFileSync(path.join(root, 'pages/product-detail/index.js'), 'utf8')
  const miniAuth = fs.readFileSync(path.join(root, 'utils/auth.js'), 'utf8')
  ...
  assert.match(web, /可填写吊牌、进货单或供应商货号/)
  assert.match(web, /货号未填写/)
  assert.doesNotMatch(web, /内部流水号|系统编号/)
  assert.match(web, /statusFilter=ref\('active'\)/)
  assert.match(web, /重新启用/)
  assert.match(web, /editing\.id&&canDeleteProducts/)
  assert.match(web, /editingDeletionEligibility\?\.canDelete/)
  assert.match(web, /该商品尚无经营记录，将永久删除商品档案，此操作不可撤销。/)
  assert.doesNotMatch(web, /@click="requestDelete\(p\)"/)
  assert.match(web, /@click="requestDelete\(editing\)"/)
  assert.doesNotMatch(web, /强制删除/)
  assert.match(miniForm, /maxlength="80"/)
  assert.match(miniForm, /货号（选填）/)
  assert.match(miniForm, /可填写吊牌、进货单或供应商货号/)
  assert.doesNotMatch(miniForm, /内部流水号|系统编号|systemCode/)
  assert.match(miniLogic, /serverSync\.updateProductProfile/)
  assert.match(miniDetail, /编辑商品/)
  assert.match(miniDetailLogic, /pages\/product-form\/index\?id=/)
  assert.match(miniDetailLogic, /auth\.canEditProducts\(\)/)
  assert.match(miniLogic, /仅店主可编辑商品资料/)
  assert.match(miniAuth, /getCurrentUser\(\)\?\.role === 'owner'/)
  assert.doesNotMatch(saleForm, /itemNumber \|\| currentProduct\.code/)
})

test('Web、小程序库存与 AI 候选只展示真实货号', () => {
  const root = path.resolve(__dirname, '..')
  ...
  const miniInventory = fs.readFileSync(path.join(root, 'pages/inventory/index.wxml'), 'utf8')
  const miniCosmetics = fs.readFileSync(path.join(root, 'pages/cosmetics/index.wxml'), 'utf8')
  const saleForm = fs.readFileSync(path.join(root, 'pages/sale-form/index.wxml'), 'utf8')
  const purchaseForm = fs.readFileSync(path.join(root, 'pages/purchase-form/index.wxml'), 'utf8')
  ...
  const userInterfaces = [webProducts, webInventory, miniStock, miniInventory, miniCosmetics, saleForm, purchaseForm, normalAi, purchaseAi]
  userInterfaces.forEach(source => assert.doesNotMatch(source, /内部流水号|未填写货号/))
  assert.match(webProducts, /\['商品名称','货号','业务类型'/)
  assert.doesNotMatch(webProducts, /function exportCSV\(\)[^\n]*p\.code/)
  assert.match(webInventory, /item\.itemNumber\|\|'货号未填写'/)
  assert.match(miniStock, /item\.itemNumber \? '货号 '/)
  assert.match(miniStock, /'货号未填写'/)
  assert.match(normalAi, /item\.itemNumberText/)
  assert.doesNotMatch(normalAi, /item\.internalCodeText|item\.code/)
  assert.ok(normalAi.indexOf('candidate-item-number') < normalAi.indexOf('candidate-name'))
  assert.match(purchaseAi, /candidate\.itemNumber \? '货号 '/)
  assert.doesNotMatch(purchaseAi, /candidate\.code|inactiveMatch\.code/)
  assert.ok(purchaseAi.indexOf("candidate-code item-number") < purchaseAi.indexOf('candidate-name'))
})

test('普通商品 CSV 不默认导出内部流水号', () => {
  const web = fs.readFileSync(path.resolve(__dirname, '../admin/src/views/ProductsView.vue'), 'utf8')
  const exportLine = web.split('\n').find(line => line.includes('function exportCSV')) || ''
  assert.match(exportLine, /\['商品名称','货号','业务类型','分类','库存','成本价','售价','状态'\]/)
  assert.doesNotMatch(exportLine, /内部流水号|p\.code/)
})
