...
const filtered=computed(()=>items.value.filter(item=>!query.value||`${item.name} ${item.itemNumber||''}`.toLowerCase().includes(query.value.toLowerCase())))
...
<template>
  <section class="business-page">
    ...
    <article v-if="tab==='stock'" class="data-card"><div class="filterbar"><div class="filters"><label><span class="material-symbols-outlined">search</span><input v-model="query" placeholder="搜索商品/货号" /></label></div><span class="muted">{{filtered.length}} 个商品</span></div><div class="data-table-wrap"><table class="data-table"><thead><tr><th>商品</th><th>货号</th><th>分类</th><th>货位</th><th class="right">聚合库存</th><th class="right">阈值</th><th class="right">库存成本</th><th>库存状态</th></tr></thead><tbody>
      <tr v-for="item in filtered" :key="item.id"><td><b>{{item.name}}</b></td><td><div class="mono" :class="{muted:!item.itemNumber}">{{item.itemNumber||'货号未填写'}}</div></td><td>{{item.businessType}} · {{item.category}}</td><td>{{item.location||'未设置'}}</td><td class="right mono">{{item.stock}}</td><td class="right mono">{{item.lowStockThreshold}}</td><td class="right mono">{{canViewCost?money(item.stock*item.costPrice):'无权限'}}</td><td><span class="status-pill" :class="item.stock===0?'danger':item.stock<=item.lowStockThreshold?'warning':'success'">{{item.stock===0?'缺货':item.stock<=item.lowStockThreshold?'库存偏低':'正常'}}</span></td></tr>
