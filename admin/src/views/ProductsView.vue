...
<template>
  <section class="products-page">
    ...
    <div v-if="pendingConfirm" class="dialog-backdrop action-confirm-backdrop" @mousedown.self="closeConfirm"><section class="business-dialog action-confirm" role="dialog" aria-modal="true" :aria-labelledby="`product-confirm-${pendingConfirm.kind}`"><h3 :id="`product-confirm-${pendingConfirm.kind}`">{{pendingConfirm.kind==='delete'?'永久删除商品':'确认停用商品'}}</h3><p>{{pendingConfirm.kind==='delete'?'该商品尚无经营记录，将永久删除商品档案，此操作不可撤销。':'停用后，该商品将不再参与卖货、拿货和普通 AI 匹配，历史记录仍会保留。确定停用吗？'}}</p><div class="dialog-actions"><button type="button" class="secondary" @click="closeConfirm">取消</button><button type="button" class="danger-button" :disabled="saving||deletingId" @click="confirmPendingAction">{{pendingConfirm.kind==='delete'?'永久删除':'确认停用'}}</button></div></section></div>
  </section>
</template>
...
.action-confirm{box-sizing:border-box;width:min(440px,100%);overflow-x:hidden;text-align:center}
